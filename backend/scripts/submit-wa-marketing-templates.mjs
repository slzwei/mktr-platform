 ...
 *   --all            read-only: EVERY template on the WABA with its status,
 *                    category and language — the only way to see templates this
 *                    script never submitted (the _v2 UTILITY twins, the approved
 *                    originals). Meta's 2026 "approve-and-flip" recategorises
 *                    silently, so category is a thing to re-read, not remember.
 ...
 * one of those the category and language here are the facts that decide whether
 * the send survives the marketing frequency cap (131049) and resolves at all.
 */
function printAllTable(existing) {
  ...
  console.log(`\n  ${rows.length} templates · UTILITY ones are exempt from the per-user marketing cap.`);
}
