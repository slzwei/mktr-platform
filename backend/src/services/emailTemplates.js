...
export function getRoleInviteSubject({ companyName = 'MKTR', roleLabel = 'User' } = {}) {
  return `You are invited to join ${companyName} as a ${roleLabel}`;
}

export function getRoleInviteEmail({
  firstName = 'there',
  inviteLink,
  companyName = 'MKTR',
  companyUrl,
...
<html>
  <head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover" />
    <title>${companyName} ${roleLabel} Invitation</title>
    <style>
      body { margin:0; padding:0; background-color:#f6f9fc; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol', sans-serif; color:#0f172a; }
      a { color: inherit; text-decoration: none; }
      .container { width:100%; background:#f6f9fc; padding: 24px 0; }
    ...
    <div class="container">
      <div class="card">
        ...
        <div class="content">
          <p class="greeting">Hi ${firstName},</p>
          <p class="message">
            You have been invited to join <strong>${companyName}</strong> as a <strong>${roleLabel}</strong>. Click the button below to accept your invitation and complete your registration.
          </p>
          <div class="cta-wrap">
            <a class="button" href="${safeLink}" target="_blank" rel="noopener noreferrer">Accept Invitation</a>
          </div>
...
export function getRoleInviteText({ firstName = 'there', inviteLink, companyName = 'MKTR', expiryDays = 7, roleLabel = 'User' }) {
