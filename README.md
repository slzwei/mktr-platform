...
- **Customer-facing URL helpers always point to `redeem.sg`** regardless of which brand is rendering, so the admin's "Copy Link"/QR/preview surfaces produce clean `redeem.sg` links with no redirect hop: `customerPublicUrl(path)`, `customerLeadCaptureUrl(campaignId, params)`, `customerPreviewUrl(slug)`, `publicTrackingUrl(slug)`, `publicShareUrl(slug)`.
