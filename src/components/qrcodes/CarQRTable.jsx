...
export default function CarQRTable({ qrTags, onRefresh, refreshing }) {
 ...
 All Car QR Codes ({qrTags.length})
 </CardTitle>
 <div className="flex items-center gap-2 w-full max-w-md">
 <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search car QRs..."/>
 ...
 {search ? `"${search}"is not found` : 'No car QR codes found.'}
 </TableCell>
 </TableRow>
 ) : filtered.map((qr) => (
 ...
 <TableCell>
 <div className="font-medium text-foreground">{qr.car?.plate_number || (qr.label ? `Car: ${qr.label}` : `Car ID: ${qr.carId}`)}</div>
 {qr.car && (qr.car.model || qr.car.make) ? (
 <div className="text-sm text-muted-foreground">{qr.car.make} {qr.car.model}</div>
 ) : null}
 ...
 title="Delete car QR?" description={
 <>
 This will permanently delete the car QR for
 <span className="font-semibold mx-1">
 {qr.car?.plate_number || qr.carId || '-'}
 </span>
