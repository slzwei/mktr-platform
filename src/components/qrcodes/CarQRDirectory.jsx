...
export default function CarQRDirectory({ campaign, onAssigned }) {
 ...
 {success && (
 <div className="text-success bg-success/10 p-3 rounded">{success}</div>
 )}
 ...
 <Input
 className="pl-10" placeholder="Search by plate number..." value={filters.search}
 ...
 <AlertDialogAction onClick={bulkAssign} disabled={assigning || !selectedCampaignId} className="bg-primary hover:bg-primary/90">
 {assigning ? <Loader2 className="w-4 h-4 mr-2 animate-spin"/> : null}
 Confirm
 </AlertDialogAction>
 </AlertDialogFooter>
