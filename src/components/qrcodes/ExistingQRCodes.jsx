...
export default function ExistingQRCodes({ qrTags, loading, onRefresh }) {
 ...
 <div className="text-xs text-muted-foreground truncate" title={qr.slug}>
 Slug: {qr.slug}
 </div>
 {qr.slug && (
 ...
 <span className="text-warning">No agent assigned</span>
 )}
 </div>
 </TableCell>
 <TableCell>
 ...
 <span className="font-semibold text-sm text-primary">{qr.scanCount ?? 0}</span>
 </div>
 <div className="flex items-center gap-1">
 <span className="text-xs text-muted-foreground">Unique:</span>
 <span className="font-medium text-xs text-success">{qr.uniqueScanCount ?? 0}</span>
 </div>
 </div>
 </TableCell>
 <TableCell>
 ...
 <Button
 variant="outline" size="sm" onClick={() => openEditDialog(qr)}
 ...
 {editError && (
 <div className="text-destructive bg-destructive/10 p-2 rounded text-sm">{editError}</div>
 )}
