...
export default function PromotionalQRForm({ campaign, onQRGenerated }) {
 ...
 <h3 className="font-semibold text-info mb-2">About Promotional QR Codes</h3>
 ...
 <Input
 id="promo-label" value={formData.label}
 onChange={(e) => setFormData({...formData, label: e.target.value})}
 placeholder="e.g., Orchard Road Booth" maxLength={100}
 required
 aria-required="true" />
 </div>
 ...
 <Textarea
 id="promo-tags" value={formData.tagsInput}
 onChange={(e) => setFormData({...formData, tagsInput: e.target.value})}
 placeholder="Facebook, Instagram, Street Team" rows={3}
 maxLength={250}
 />
 <p className="text-sm text-muted-foreground">Multiple tags are supported; a single QR will include all tags for reporting.</p>
 </div>
 ...
 <Textarea
 id="promo-description" value={formData.description}
 onChange={(e) => setFormData({...formData, description: e.target.value})}
 placeholder="Additional notes about this QR code..." rows={3}
 maxLength={250}
 />
 </div>

 <Button
 type="submit" disabled={loading || !formData.label.trim()}
