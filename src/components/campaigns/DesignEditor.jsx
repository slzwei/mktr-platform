...
export default function DesignEditor({ campaign, onSave }) {
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] gap-0">
      {/* Editor Panel */}
      <div className={`${panelOpen ? 'w-[380px] min-w-[380px]' : 'w-0 min-w-0 overflow-hidden'} transition-all duration-300 flex flex-col border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900`}>
        {/* Tab Bar */}
        ...
        <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
          <Button
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 dark:disabled:bg-blue-800"
          >
            {saving ? (
              ...
              <>
                Save Design
                {hasUnsavedChanges && <span className="ml-2 w-2 h-2 bg-orange-400 rounded-full animate-pulse" />}
              </>
            )}
          </Button>
          <div className="mt-1.5 text-center">
            {saving ? (
              <p className="text-xs text-blue-600 dark:text-blue-400">Saving changes...</p>
            ) : hasUnsavedChanges ? (
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">Changes are saved manually</p>
            )}
          </div>
        </div>
      </div>
      ...
      <button
        ...
        className="flex items-center justify-center w-6 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors border-r border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/30"
        title={panelOpen ? 'Collapse panel' : 'Expand panel'}
      >
        {panelOpen ? <PanelLeftClose className="w-3.5 h-3.5 text-gray-400" /> : <PanelLeft className="w-3.5 h-3.5 text-gray-400" />}
      </button>

      {/* Preview */}
      <div className="flex-1 min-w-0 p-4 bg-gray-50 dark:bg-gray-900">
