...
export function getBackgroundClass(design) {
 ...
 return { className: 'bg-paper', style: {} };
 ...
 case 'solid': // Legacy
 return { className: 'bg-muted', style: {} };
 case 'pattern': // Legacy
...
function MediaHeader({ design }) {
 ...
 if (youtubeUrl) {
 return (
 <div className="w-full relative bg-foreground" style={{ aspectRatio: '16/9' }}>
 <iframe
 src={youtubeUrl}
 title="Campaign Video" className="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen
 loading="lazy" />
 </div>
 );
 }
 ...
 alt="Campaign Header" loading="lazy" decoding=" async" className="w-full h-full object-cover" />
...
export default function LeadCaptureLayout({ design = {}, maxWidth, showTrustFooter = false, children }) {
 ...
 <div className="flex items-center justify-center gap-4 opacity-60 grayscale transition-colors hover:grayscale-0 hover:opacity-100">
 <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground font-medium bg-card px-2 py-1 rounded-full border border-border">
