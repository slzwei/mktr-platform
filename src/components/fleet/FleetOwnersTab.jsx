...
export default function FleetOwnersTab({ fleetOwners, fleetOwnerUsers, cars }) {
 ...
 <Input
 placeholder="Search fleet owners..." value={searchTerm}
...
const FleetOwnerRow = React.memo(function FleetOwnerRow({
 ...
 <p className="text-sm text-muted-foreground">ID: {owner.id.slice(-8)}</p>
 {owner.uen && (
 <p className="text-xs text-muted-foreground">UEN: {owner.uen}</p>
 )}
 </div>
 </div>
 </TableCell>
 ...
 className={owner.status === 'active' ? 'bg-success/15 text-success' : 'bg-destructive/15 text-destructive'}
 >
 {owner.status || 'active'}
 </Badge>
 </TableCell>
 ...
 <Button
 variant="ghost" size="sm" className="text-success hover:text-success" onClick={() => onApprove(foUser.id)}
 >
 Approve
 </Button>
 <Button
 variant="ghost" size="sm" className="text-destructive hover:text-destructive/80" onClick={() => onReject(foUser.id)}
 >
 Reject
 </Button>
 </div>
 ) : (
 <span className="text-sm text-muted-foreground">{foUser?.approvalStatus || 'approved'}</span>
 )}
 </TableCell>
 ...
 <div className="flex gap-2">
 <Button
 variant="ghost" size="sm" onClick={() => onEdit(owner)}
 >
 <Edit className="w-4 h-4"/>
 </Button>
 <Button
 variant="ghost" size="sm" onClick={() => onDelete(owner.id)}
