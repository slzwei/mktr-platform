...
export const AssignCampaignDialog = ({ device, open, onClose, onAssign }) => {
 ...
 const available = campaignsList.filter((c) => c.status === 'active' && c.type === 'brand_awareness');
 ...
 <div className="border rounded-md p-1 h-60 overflow-y-auto bg-muted/50 space-y-1">
 {loading ? (
 <div className="p-4 text-center text-xs text-muted-foreground">Loading...</div>
 ) : campaigns.length === 0 ? (
 <div className="p-4 text-center text-xs text-muted-foreground">No active PHV campaigns found.</div>
 ) : (
 campaigns.map((c) => (
 <div
 key={c.id}
 className={`flex items-start space-x-3 p-2 rounded hover:bg-card hover:shadow-sm transition-colors border border-transparent hover:border-border ${selectedIds.includes(c.id) ? 'bg-card border-border shadow-sm' : ''}`}
 >
 <Checkbox
 id={`c-${c.id}`}
 ...
 <p className="text-[10px] text-muted-foreground">
 Only 'Brand Awareness' (PHV) campaigns are shown. Regular 'Lead Gen' campaigns cannot be assigned to
 tablets.
 </p>
 </div>
 ...
 <Button
 type="button" variant="ghost" size="sm" onClick={() => setSelectedIds([])}
