...
- **Discover's "Restrict Google categories"** is NOT free text: the Maps actor validates `categoryFilterWords` against a closed, all-lowercase enum of ~4k Google categories and 400s the whole run on one bad value — Title case alone is enough ("Gymnastics center" was rejected in prod, 07-27). Every word (operator, saved category, AI suggestion) is folded onto that enum by `backend/src/services/redeemOps/discovery/placeCategories.js`; the mirrored list is generated — refresh with `node backend/scripts/refresh-place-categories.mjs`, never by hand.
- **Cadences**: Outreach-style sequencing engine; AI drafts live (both flags on). Authoring home `/redeem-ops/cadences` + editor are `tasks.manage` (BDM/outreach exec, not just admins); row actions are creator-or-admin (`canAuthorRow`), mirrored in CadenceStudio/CadenceEditorPage. P2 = pool/Discover claim-and-enroll. **The engine NEVER auto-skips a step** (2026-08-11): a step it can't prepare (no email/phone/handle/outlet, suppressed, bad template) parks the run ON that step (`pausedReason='missing_info'` + `blockedReason`) until the rep fixes the record (auto-resumes), hits `skip-step` (the only way past without an outcome), or stops.
