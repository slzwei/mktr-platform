...
export default function AdminAgentGroups() {
 ...
 const filteredLyfeAgents = lyfeAgents
 .filter(
 (a) => (a.name || '').toLowerCase().includes(agentSearch.toLowerCase()) || (a.phone || '').includes(agentSearch)
 )
 .slice(0, 10);
 ...
 <Input
 className="pl-9" value={agentSearch}
 ...
 {filteredLyfeAgents.length > 0 ? (
 filteredLyfeAgents.map((agent) => (
 <button
 key={agent.id || agent.phone}
 type="button" className="w-full text-left px-3 py-2 hover:bg-accent text-sm flex justify-between" onClick={() => addAgent(agent)}
 >
