...
function statusOf(u) {
  ...
  if (!u.lastLogin && u.invitationToken) return { label: 'Invited', tone: 'accent' };
...
export default function AdminV2Users() {
  ...
  return (
    <div>
      <PageHeader title="Users" meta={`${fmtNumber(staff.data?.total ?? 0)} BACK-OFFICE ADMINS`}>
        <button type="button" className="av2-btn av2-btn--primary" onClick={() => setInviting(true)}>+ Invite admin</button>
      </PageHeader>

      <div className="av2-card" style={{ overflow: 'hidden' }} role="grid" aria-label="Staff users">
        ...
        {!staff.isLoading && !staff.isError && rows.length === 0 && (
          <EmptyState title="No staff users" hint="Invite the first admin to share the console." />
        )}
        ...
        {rows.map((u) => {
          ...
          return (
            <div key={u.id} className="av2-row" style={{ cursor: 'default' }} role="row">
              ...
              <span role="gridcell" style={{ width: 90, flex: 'none' }}><Chip tone="accent">Admin</Chip></span>
              <span role="gridcell" style={{ width: 100, flex: 'none' }}><Chip tone={st.tone}>{st.label}</Chip></span>
              <span role="gridcell" className="av2-mono" style={{ width: 110, flex: 'none', fontSize: 10.5, color: 'var(--ink-3)', textAlign: 'right' }}>
                {u.lastLogin ? fmtRelative(u.lastLogin) : st.label === 'Invited' ? 'never' : '—'}
              </span>
              <span role="gridcell" className="av2-mono" style={{ width: 110, flex: 'none', fontSize: 10.5, color: 'var(--ink-3)', textAlign: 'right' }}>{fmtDate(u.createdAt)}</span>
            </div>
          );
        })}
      </div>
