...
describe('LeadCapture', () => {
 ...
 // The agree-all block's wording-era label must survive handleSubmit's
 // basePayload rebuild + null/empty filter — without it the backend stamps the
 // LEGACY copy/hash as evidence for a person who saw the NEW block.
 it('forwards consent_copy_version (and the consent booleans) to /prospects', async () => {
 renderPage();
 await waitFor(() => expect(screen.getByTestId('signup-form')).toBeInTheDocument());
 const user = userEvent.setup();
 await user.click(screen.getByText('Submit'));
