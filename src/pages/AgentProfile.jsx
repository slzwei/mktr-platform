...
export default function AgentProfile() {
 ...
 toast.error('Please ensure your new password and confirmation match.');
 return;
 }
 ...
 toast.error(error.message || 'Please check your current password.');
 } finally {
 ...
 onChange={handleProfileChange}
 className="pl-9 bg-muted text-muted-foreground" disabled={isGoogleUser} // Lock email for Google users for simplicity/security
 ...
 <Input
 id="dateOfBirth" name="dateOfBirth" type="date" value={formData.dateOfBirth}
 onChange={handleProfileChange}
 className="pl-9" />
 </div>
 </div>
 ...
 className={`pl-20 ${phoneChanged && otpState !== 'verified' ? 'border-warning/30 focus-visible:ring-warning/30' : ''}`}
 maxLength={8}
 ...
 {!phoneChanged && (
 <div className="flex items-center gap-1.5 text-success text-xs font-medium bg-success/10 px-2 py-0.5 rounded-full">
 ...
 <div className="flex items-center gap-1.5 text-warning text-xs font-medium bg-warning/10 px-2 py-0.5 rounded-full">
 Using New Number
 </div>
 )}
 </div>
 ...
 {phoneChanged && otpState === 'verified' && (
 <Button
 type="button" variant="ghost" size="icon" aria-label="Undo phone change" onClick={() => {
 setOtpState('idle');
 setFormData((p) => ({ ...p, phone: originalPhone }));
 }}
 title="Undo changes" >
 ...
 <label className="text-sm font-medium text-muted-foreground block mb-2">Enter Verification Code</label>
 <div className="flex gap-2">
 <Input
 className="max-w-[140px] tracking-widest text-center font-mono text-lg" placeholder="000000" maxLength={6}
 value={otp}
 ...
 <Button
 type="button" variant="ghost" disabled={resendCooldown > 0}
 onClick={handleSendOtp}
 className="text-xs text-muted-foreground hover:text-foreground" >
 {resendCooldown > 0 ? `Resend in ${resendCooldown}s` : 'Resend Code'}
 </Button>
 </div>
 <p className="text-xs text-muted-foreground mt-2">We sent a 6-digit code to +65 {formData.phone}.</p>
 </motion.div>
 )}
 ...
 <Button
 type="submit" form="profile-form" disabled={submittingProfile || (phoneChanged && otpState !== 'verified')}
 >
 ...
 <Input
 type="password" id="currentPassword" value={passwordData.currentPassword}
 ...
 <Input
 type="password" id="newPassword" value={passwordData.newPassword}
 ...
 <Input
 type="password" id="confirmPassword" value={passwordData.confirmPassword}
