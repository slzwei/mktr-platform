...
export default function Onboarding() {
 ...
 <button
 type="button" onClick={() => goToStep(i)}
 aria-label={`Go to ${s.label}`}
 className={`${i <= maxVisitedStep ? 'cursor-pointer hover:opacity-80' : 'cursor-not-allowed'} ${i <= step ? 'bg-foreground' : 'bg-muted-foreground'} w-3 h-3 rounded-full`}
 disabled={i > maxVisitedStep}
 />
 <button
 type="button" onClick={() => goToStep(i)}
 className={`mt-2 text-muted-foreground text-sm md:text-base font-medium ${i <= maxVisitedStep ? 'hover:underline cursor-pointer' : 'cursor-not-allowed'}`}
 aria-label={`Go to ${s.label}`}
 disabled={i > maxVisitedStep}
 >
 {s.label}
 ...
 {i < steps.length - 1 && (
 <div className={`${i < step ? 'bg-foreground' : 'bg-muted-foreground'} h-0.5 w-16 sm:w-24 mx-2`}></div>
 )}
 </Fragment>
 ))}
 </div>
 {step === 1 && (
 <div className="text-sm text-muted-foreground">Signing up as a <span className="font-medium">{roleLabel}</span></div>
 )}
 ...
 <div
 className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${step * 100}%)` }}
 >
 {/* Step 1 */}
 <StepProfile
 role={role}
