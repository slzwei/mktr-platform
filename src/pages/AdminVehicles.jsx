...
export default function AdminVehicles() {
 ...
 <Button
 variant="ghost" size="sm" className="text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => handleDeleteVehicle(vehicle.id)}
 >
 ...
 ? 'bg-primary/10 text-primary border-info/30 '
 ...
 {(vehicle.masterDevice || vehicle.slaveDevice) && (
 <Button
 variant="outline" size="sm" className="text-warning hover:text-warning" onClick={() => handleUnpair(vehicle.id)}
 >
 ...
 .map((device) => (
 <div key={device.id} className="p-3 border rounded-lg bg-muted/50">
 ...
 ? 'bg-success/10 text-success border-success/30 mt-2'
 : getDeviceStatus(device).color === 'blue'
 ? 'bg-primary/10 text-primary border-info/30 mt-2'
 : 'bg-muted text-muted-foreground border-border mt-2'
 }
 >
 ...
 <Input
 placeholder="e.g. SGX1234A" value={newCarplate}
 ...
 <input
 type="checkbox" checked={selectedCampaignIds.includes(campaign.id)}
 onChange={(e) => {
 if (e.target.checked) {
 ...
 <Badge variant="outline" className="text-xs bg-plum/10 text-plum border-plum/30">
 {typeLabel}
 </Badge>
 </div>
 ...
 <span
 className={`${campaign.status === 'active' ? 'text-success' : 'text-muted-foreground'}`}
 >
 {campaign.status}
 </span>
 </div>
 ...
 <Button
 variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => navigate(`/admin/campaigns/${campaign.id}/edit`)}
 title="Edit Campaign" >
