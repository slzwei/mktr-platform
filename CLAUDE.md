...
| **Campaign Studio** (the PERMANENT campaign design surface — design_config v2 twins, v2 renderer `src/components/campaignPage/` reusing the funnel, full-viewport editor + AI assist at `/admin/campaigns/:id/studio`; classic DesignEditor survives ONLY for guided_review; backend `DESIGN_CONFIG_V2_WRITES_ENABLED` = emergency brake; per-campaign migration/rollback runbook) | `docs/reference/campaign-studio-rollout.md` (rollout/rollback) · `docs/plans/campaign-studio-implementation-prompt.md` (build history) · twins `src/lib/designConfigV2.js` ↔ `backend/src/utils/designConfigV2.js` (+ `designConfigV2Clamp.js`) |
