...
- **Code:** `backend/src/services/redeemedAudienceService.js` — select non-`call_bot` prospects → consent-gate → hash email+phone via `piiHashing` → batch ≤10k → `POST /{audience_id}/users` with an `Authorization: Bearer` header (never `?access_token=`). Run by `backend/scripts/sync-redeemed-audience.js` via `RUN_MODE=cron-redeemed-audience` (`entrypoint.sh`; `.dockerignore` allowlists the script). Cron Sentry comes from the new shared `backend/src/utils/sentryInit.js` (extracted from `server.js` so the script gets PII-scrubbed Sentry, not a bare init). 21 unit tests: `test/redeemedAudienceService.test.js`.
...
- **Deploy status (2026-06-22):** code written + tested, **not yet deployed**. Remaining: set the env vars on a daily **Render Cron Job** (`RUN_MODE=cron-redeemed-audience`), probe the token + `usersreplace`, then flip `REDEEMED_AUDIENCE_SYNC_ENABLED=true`. Full design + open decisions: `docs/plans/meta-redeemed-audience-sync.md`.
