...
| **RSVP pages** (`rsvp.redeem.sg/{slug}` — admin-designed event pages, NOT campaigns/leads: own tables `rsvp_events`/`rsvp_responses`, own layout twin `src/lib/rsvpLayout.js` ↔ `backend/src/utils/rsvpLayout.js`, one renderer for designer preview + public page, third static site `VITE_SURFACE=rsvp`, flags `RSVP_ENABLED` + `VITE_RSVP_ENABLED`) | `docs/plans/rsvp-pages.md` (plan + Codex log + delivery notes) · hosting in `docs/reference/brand-and-hosting.md` |
