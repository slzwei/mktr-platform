...
**Down-funnel CAPI events (reverse path — `QualifiedLead` + `ClosedWon`):** lead quality is only known in Lyfe, so the signal travels *back* from Lyfe to MKTR. When an agent advances a lead's `status` to `qualified`/`won`, a Supabase trigger (`leads_notify_mktr_outcome`, reference SQL in `docs/plans/lyfe-leads-outcome-webhook.sql`, applied in the separate `lyfe-app` repo) HMAC-POSTs to **`POST /api/integrations/lyfe/lead-outcome`** (`backend/src/routes/lyfeLeadOutcome.js` + controller, same HMAC + 5-min-window auth as `users-webhook`). `backend/src/services/leadOutcomeService.js` looks up the prospect (`external_id === prospect.id`), guards against re-firing via a `sourceMetadata.capi.{qualifiedAt|wonAt}` marker (first transition only), and fires `QualifiedLead`/`ClosedWon` with a deterministic `event_id` (`qualified:{id}`/`won:{id}`) back-dated to the status change. Event names are env-overridable (`META_EVENT_QUALIFIED`/`META_EVENT_WON`, so `won` can later become `Purchase`). Secret: `LYFE_LEAD_OUTCOME_SECRET`. **Measure first** — keep the ad set optimizing on `Lead`; only switch optimization to a `QualifiedLead` Custom Conversion once volume reaches ~30–50/week (enforce a 24–72h qualification SLA so events land in the 7-day click window). Note: there is no negative/"NotPR" Meta signal — non-SC/PR clickers fire `ViewContent` but never `Lead`, which is already the implicit negative the `Lead`-optimized ad set learns from.
