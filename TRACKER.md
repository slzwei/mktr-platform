...
| Studio teardown (Campaign Studio follow-up) | DONE | The Studio is the PERMANENT design surface: `VITE_CAMPAIGN_STUDIO_ENABLED` removed (route always registered; workspace Design tab = Open-in-Studio for every non-guided_review campaign), standalone `/AdminCampaignDesigner` page deleted (legacy URL redirects to the workspace Design tab), classic `DesignEditor` survives ONLY as the guided_review designer (its v2 notice always links to the Studio). `useStudioDoc` message-regex draw-422 fallback removed (typed codes deployed). Malformed campaign `:id` params now 404 cleanly via `router.param` guard (was a Postgres uuid-cast 500 → scary "Database Error" panel — found live during the rollout). Backend `DESIGN_CONFIG_V2_WRITES_ENABLED` kept as the emergency brake. |
...
| Meta Lead Ads native ingestion | DONE (dark) | 2026-08-06, `docs/plans/meta-lead-ads-native-pipe.md`. Flag `META_LEAD_ADS_ENABLED` (default OFF; prod boot fails on partial secrets). `POST /api/meta/webhook` (sig-verified, raw-body allowlisted) → durable inbox `meta_leadgen_events` (permanent leadgenId dedupe, SKIP LOCKED worker, 2^n-min backoff → dead-letter, admin retry endpoint) → Graph fetch → standard pipeline: normalized phone, consent ledger source `meta_lead_ad` (registry era `2026-08-06-meta-leadgen-v1`, DB CHECK extended in 115), DNC gate, quota/held, TRANSACTIONAL outbox delivery (persistEventDeliveries in-tx + flush). Routing: `meta_form_mappings` (form→campaign, optional direct-agent QR) + `meta_pages` allowlist (AES-GCM-sealed tokens, env fallback bound to META_PAGE_ID); unmapped/undeliverable → `[Meta] Unmapped` held pool (slug `meta-unmapped`, bootstrap-ensured, boot-fatal when flag on). Migrations 112-115. Tests: 42 unit + 6 integration (incl. frozen receiver payload contract) green. |
