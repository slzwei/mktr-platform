...
- WhatsApp: `canWhatsAppProspect(prospect, { purpose })` — `purpose:'transactional'` (reservation pass, voucher, resend) ignores marketing suppressions but respects `erasure`; `purpose:'marketing'` requires full `canMarketTo`. This resolves the `whatsappService.js:39-47` ambiguity explicitly instead of inheriting it. **[SHIPPED 2026-07-21 — the 3sites migration implemented this whole section: audience sync reads `getMarketableGrantMap` (grant+verified, phone-keyed) + the suppression set; submit-time AND delayed CAPI derive em/ph from `canMarketTo` at dispatch (`ctx.marketingConsent`); D2 resolved as transactional.]**
