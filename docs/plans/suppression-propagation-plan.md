...
# v3 addendum — resubscribe lift (Shawn's Reading-2 decision, 2026-07-22)

**Decision:** a fresh, OTP-VERIFIED, brand-scope-era (agree-all) contact grant LIFTS an existing `unsubscribe`-reason suppression. Latest-explicit-consent-wins. Only `unsubscribe` lifts — `complaint`/`admin` (not the person's own act) and `erasure` (identity latch; erased people get fresh identities anyway) never auto-lift. Rule goes into the counsel pack (tracker "legal") for the already-planned review.

## Emitter (mktr-platform)

1. **Lift hook** in `recordCaptureConsentEventsTx`'s savepoint: when `era.scope==='brand' && contact===true && verified===true` → findOne ConsumerSuppression (consumerId, channel 'all', reason 'unsubscribe') → if present: destroy it + append a ledger event `{kind:'contact', granted:true, campaignId:null, source:'resubscribe', version: era label, verified:true, metadata:{lift:{reason,source,suppressedAt}}}` — atomic with the capture's grant events; savepoint failure leaves the suppression (fail-safe = stays suppressed). Model `source` validator gains `'resubscribe'` (no DB CHECK exists).
2. **Readers self-correct**: canMarketTo/audience/CAPI/WA need zero changes (row gone → ledger rules; the fresh verified global grant authorizes).
3. **Projection state machine** (migration 086 + model): `suppression_propagations` gains `state` ('suppressed'|'lifted', default 'suppressed') + `deliveredState` (nullable); backfill deliveredState='suppressed' where queuedAt IS NOT NULL. Needs-queue = `deliveredState IS DISTINCT FROM state` (replaces queuedAt-null) OR delivered-and-failed/purged. CAS claim sets `deliveredState = state` atomically (`SET "deliveredState" = state ... RETURNING state, reason, "occurredAt"`), event type chosen from the RETURNED state: suppressed→lead.suppressed, lifted→lead.unsuppressed (new builder; payload `data:{lead:{externalId}, unsuppression:{schemaVersion:1, scope:'marketing', reason, occurredAt}}`).
4. **Reconciler derivation v3**: targets = suppressed-or-erased consumers UNION consumers with existing pairs. Active non-erasure suppression → marketing lane suppressed (as today). NO suppression + existing marketing pairs → `lifted` with occurredAt from the LATEST `source='resubscribe'` ledger event (no event ⇒ NO flip — a manual row delete stays manual; evidence-driven only). `'all'` lane is a latch — never flips. Lift pairs queue only to subscribers whose events include `lead.unsuppressed` (skip silently otherwise).
5. **New-lead correctness needs no lift delivery**: a lifted person's next signup projects no new pairs (no suppression) and downstream stamps nothing (tombstone cleared by the lift delivery for OLD leads; a NEW lead's external_id never had a tombstone). wa_intro fires again from the first post-resub signup.
6. **Bootstrap**: both destination flags now append BOTH `lead.suppressed` + `lead.unsuppressed`. DEPLOY ORDER: consumers first (handlers live before any lift can be sent), then this PR.
7. Cycle safety: re-unsubscribe recreates the suppression row → pairs flip back → redelivery with newer occurredAt; downstream watermark (strictly-newer wins for scope 'marketing') makes out-of-order/repair deliveries idempotent in both directions.

## Consumers

- **lyfe**: migration — `ALTER TYPE lead_activity_type ADD VALUE 'resubscribed'`; RPC `apply_mktr_lead_unsuppression(source_name, external_id, occurred_at, delivery_id)`: tombstone exists AND scope='marketing' AND p_occurred_at > tombstone.occurred_at → DELETE tombstone + clear lead do_not_contact columns WHERE scope='marketing' (never 'all') + activity when changed; idempotent + out-of-order-safe. EF: allowlist + handler (validate `data.unsuppression`). Badge/gate clear automatically (columns null).
- **mktr-leads**: same shape + processed_deliveries claim first; free-text activity type.

## Tests

Lift matrix (verified brand-era lifts; unverified / legacy-era / admin / complaint / erasure never), ledger evidence snapshot, cycle (suppress→lift→resuppress: pair state + delivery sequence + watermarks), evidence-driven-only derivation (manual delete ⇒ no flip), 'all' latch, subscriber-without-unsuppressed skip, migration backfill, requiredEvents both-events-under-flag, full regression of the 15-test propagation suite + consentLedger + erasure + webhooks + migrations.

## v3 Codex round-1 disposition (verdict "rethink" → redesigned)

Folded: #1 prod CHECKs (086 recreates chk_ce_source + chk_sp_reason with 'resubscribe'; model validators matched; prod-parity test added), #2 consumer watermark durability (tombstones → persistent state rows, lift inserts a pre-arrival hold, suppression RPCs upgraded to strictly-newer/tie-toward-suppressed, stamping gated on state='suppressed'), #3 stale-evidence replay (lift UPDATE requires qualified ledger MAX(occurredAt) > pair.occurredAt, statement-time), #4 stale-pass races (flips carry EXISTS/NOT-EXISTS guards on live suppression rows + in-statement watermark — snapshots only nominate, statements decide), #6 qualified evidence (kind/granted/verified/campaignId-null in lateral + flip), #7 direct post-lift trigger (outerTx.afterCommit) + healing posture documented, #8 snapshot broadened to either event (INSERT arm still requires lead.suppressed), #10 rename reverted (083 duplicate frozen with an explicit test allowlist — both filenames are applied in prod; renaming either is a rollback footgun), #11 hygiene (state-pending index mirrored full in tests, 084 down() drops idx_de_phone_hash, comments updated), #12 six new tests (prod-CHECK parity, stale-R1 replay, equal-watermark tie, unsuppressed-only subscriber, failed-lift requeue, afterCommit trigger). Accepted/documented: #5 OTP marker not request-bound (pre-existing spine semantics; counsel-pack note + future one-time-credential hardening), #9 ms-clock ordering (ties resolve toward suppressed at every layer — missed-lift is the worst case, never wrong un-suppression).
