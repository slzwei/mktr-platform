...
> **PR-2 BUILT 2026-07-24 (late night) — implementation notes vs the redesign notes.** All §9.1 CX9–CX13/CX18/CX19 amendments resolved as follows: provisioning went route (b) — a first-class `drawBoostProvisioningService` doing model-level writes in ONE advisory-locked tx, composing the inventory-ledger + audit primitives (the service chain's PARTNERED/staged-transition gates made route (a) impossible, documented in the file header); house partner is env-pinned with legalName fallback, NEVER auto-created (CX9); adopt validates `agent_unlock` + active offer, paused rails and wrong policies are typed 422s, `preparing` rails are promoted (CX11); offer `internalRef='draw-boost:<campaignId>'` is the orphan-adoption marker; top-up = increaseCommitted+allocate under a per-activation advisory lock at <20% of the 10k default (CX12, D1); F5 carry-forward ships in `applyLuckyDrawPolicy` (omitted key keeps stored stamp, explicit key wins); createDraw F3 fallback resolves the active rail, 422s `DRAW_BOOST_RAIL_MISSING`/`_CONFLICT`, CLI gains `--allow-no-boost`; delete-path cancels live passes in the delete tx (CX13 — bulk inherits); freeze now REQUIRES per-entrant drawTerms evidence vs the campaign's version set, excludes+counts the consentless, and returns `termsCohorts` (CX18); readiness gains `draw_boost_rail_missing` (critical live / info-warning pre-launch); F9 endsAt-inherit landed in BOTH clamp paths, idempotence-tested. **D8 (both-bounds age template) deliberately MOVED to PR-3** — it is the same surface as the age-parity lint (frontend `drawTermsTemplate.js` + AI facts + callers), and PR-2 stays backend-pure. Ensure runs ONLY on arming transitions (born-active create with compensation-to-draft, inactive→active update, draw-turning-on update, launch-state activate) — never on routine saves (CX16 trap avoided).
