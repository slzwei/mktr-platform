...
- **Meta Lead Ads ingestion REMOVED 2026-07-24** (`routes/meta.js` + controller + service + `META_APP_SECRET`/`META_PAGE_ACCESS_TOKEN`/`META_VERIFY_TOKEN`): 0 leads ever, no native forms run. The `metaLeadgenId` guards in `shouldFireCapi`/`shouldFireTikTok` remain as defence-in-depth.
