...
- `src/lib/metaPixel.js` — Pixel init + `ViewContent`/`Lead` with stable event IDs for Pixel⇄CAPI dedup; captures `_fbc` from `fbclid` and reads/synthesizes `_fbp` (`ensureFbp()` mints a first-party `_fbp` cookie when the Pixel hasn't set one yet, so server `Lead` events carry it). `shouldTrack` suppresses preview/demo/test-data routes and dev-without-test-code, and (2026-07-24) takes the CALLER-RESOLVED pixel id so a per-campaign override isn't vetoed by the build env var.
- `src/lib/pixelIds.js` (2026-07-24) — the ONE place pixel ids resolve (`campaign.metaPixelId || VITE_META_PIXEL_ID`, same for TikTok). **Loader limitation (deliberate):** `index.html`'s SDK loaders early-return without the env ids, so a campaign-ONLY pixel id cannot fire browser-side on an env-less build; both prod builds set the env ids, so overrides work today. Lazy SDK injection deferred until a second advertiser exists. Server side has no such limit (late resolution).
- `src/lib/pixelCustom.js` (2026-07-24) — `trackFunnelEvent`: the shared gated emitter for non-conversion diagnostics (`otp_sent`, `otp_verified`, `duplicate_blocked`, `draw_entry_confirmed`), fired by BOTH funnels (`CampaignSignupForm` + `MarketplaceFlow`) so they're comparable. The `dnc_*` events were deleted 2026-07-24 (registry status is not ad-platform data). `Subscribe` (waitlist) removed the same day. Lead sends NO value/currency (the old `value:0 SGD` was worse than nothing; value ships browser+server together later).
- `index.html` — base Pixel loader, gated on `VITE_META_PIXEL_ID`.
- `backend/src/services/metaCapiService.js` — fire-and-forget CAPI dispatch via the generic `sendConversionEvent(prospect, ctx, { eventName })` (thin wrappers `sendLeadEvent`/`sendCompleteRegistrationEvent`), gated by `shouldFireCapi` (skips Retell + Meta-Lead-Ads-origin prospects to avoid double-counting). Pixel id resolves LATE (`ctx.pixelIdOverride || META_PIXEL_ID`, bails `no_pixel_id` if neither — mirrors TikTok). `ctx.eventTime` back-dates `event_time` (Meta accepts ≤7 days); `ctx.actionSource` overrides `action_source` (non-website events omit `event_source_url`); `ctx.campaignIdOverride` scopes `custom_data.campaign_id`. **EMQ (2026-07-24):** hashed `fn`/`ln`/`country('sg')` ride with `em`/`ph` inside the same ledger-derived `marketingConsent` gate (`piiHashing.hashName` mirrors Meta's param-builder normalization — lowercase, strip whitespace+punctuation, KEEP digits/UTF-8).
- **`VoucherRedeemed` (2026-07-24)** — `backend/src/services/redemptionOutcomeService.js`: a voucher burned at the partner counter fires the strongest conversion in the funnel (`action_source: physical_store`, event name env-overridable `META_EVENT_REDEEMED`). Deterministic `event_id` `voucher_redeemed:{entitlementId}`; mark-on-success marker `prospect.sourceMetadata.capi.voucherRedeemed[entitlementId]`; consent/pixel/custom_data scope to the ACTIVATION's campaign (manual issuance can cross campaigns; a severed campaign link fails PII closed). Fired by `redemptionService.complete()` on EVERY redeemed resolution (fresh + replays = crash recovery) + a 6-hourly bootstrap sweep (rides `META_CAPI_ENABLED`). Owner actions after first real event: create the custom conversion in Events Manager.
- **Admin-recorded outcomes (2026-07-24)** — `prospectService.updateProspect` transitions into `qualified`/`won` fire the same `processLeadOutcome` as the Lyfe/mktr-leads webhooks (markers dedup across all three paths). The `won` transition is now atomic (status + conversionDate + Commission in one txn, System-Agent validation BEFORE mutation — fixes the stranded-won bug).
- **Meta Lead Ads ingestion REBUILT 2026-08-06** (flag `META_LEAD_ADS_ENABLED`, off by default; the 2026-07-24 removal is superseded): native instant-form leads arrive via `POST /api/meta/webhook` → durable inbox (`meta_leadgen_events`, permanent `leadgenId` dedupe) → worker fetches from Graph → standard pipeline (normalized phone, consent ledger source `meta_lead_ad`, DNC gate, quota/held, transactional outbox delivery). Page allowlist `meta_pages` (AES-GCM-sealed tokens), form routing `meta_form_mappings`, unmapped/undeliverable → `[Meta] Unmapped` held pool (slug `meta-unmapped`). The `metaLeadgenId` guards in `shouldFireCapi`/`shouldFireTikTok` now ALSO suppress double-counting for these leads. Full design: `docs/plans/meta-lead-ads-native-pipe.md`.
- Full design: `docs/plans/meta-tracking-implementation.md` · coverage hardening: `docs/plans/meta-capi-coverage-hardening.md`.
...
- **Code:** `backend/src/services/redeemedAudienceService.js` — selection + policy filtering now live in the shared **audience eligibility engine** (`audienceEligibilityService.js`, ads-centralisation §5.1: fixed filter order erased → edit-suppressed → verified-binding → grant → suppression; `AUDIENCE_POLICIES.meta` keeps the historic no-verified-binding + env-driven consent posture) — the old in-service `selectRedeemers` name was a misnomer (it selected every non-bot prospect, not redeemers) and is gone. The service keeps Meta's wire shaping (`shapeMetaAudienceRows`: synthetic-email drop, hashed multi-key rows) → batch ≤10k → `POST /{audience_id}/users` with an `Authorization: Bearer` header (never `?access_token=`), under the destination advisory lock `aud:meta:{audienceId}` it shares with the removal drainer. Tests: `test/redeemedAudienceService.test.js` + `test/audienceEligibility.test.js`. Shared `backend/src/utils/sentryInit.js` (extracted from `server.js`) gives any entrypoint PII-scrubbed Sentry.
- **Removals (ads-centralisation §5.5):** the durable outbox `audience_removals` (+ per-destination settlement watermark `audience_destination_state`) carries erasure / unsubscribe / staff-identifier-edit removals to every configured destination — Meta transport = `metaAudienceRemove` (Graph `DELETE /{audience_id}/users`, uploadBatch-mirrored payload, probe-pinned before the writer flip), Google = `removeHashedIdentifiers` (Data Manager `audienceMembers:remove` + `requestStatus:retrieve` settling). Writers gate on `AUDIENCE_REMOVAL_WRITERS_ENABLED` (legacy direct Google removal stays live while off); the drainer runs whenever any platform is removal-configured. Ops queue: rows in `needs_manual_action` (§11.4 runbook query).
